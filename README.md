...
- **拿称呼**：接 SiliconFlow API（默认 Qwen 模型），直接返回该怎么叫，比如「舅外公」「大舅子」。
- **展开解释**：结果旁边有个「更多」，点开能看到简短解释（比如「外婆的哥哥，即母亲的舅舅」）。
- **一点小规矩**：夫/妻按钮会按当前关系链智能禁用（比如不能点出「父亲的丈夫」这种不存在的组合），逻辑参考了 [mumuy/relationship](https://github.com/mumuy/relationship)。
